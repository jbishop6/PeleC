...
void ThreeBranch::build(const amrex::Geometry& geom, const int max_coarsening_level)
{
  ...
  // CORRECTED: Third branch at LEFT exit (xs - W to xs)
  const Real z_x_left = xs - W;
  const Real z_x_right = xs;
  const Real z_y_top = y_lower_lo;
  const Real z_y_bottom = z_y_top - Z;

  Print() << "\n=== THREE-BRANCH GEOMETRY (Left Exit Aligned) ===\n";
  Print() << "xs=" << xs << " (LEFT EXIT), X=" << X << ", xr=" << xr << "\n";
  Print() << "Third branch: x=[" << z_x_left << ", " << z_x_right 
          << "], y=[" << z_y_bottom << ", " << z_y_top << "]\n";
  Print() << "Should be at x ≈ " << (xs - W) << " to " << xs << "\n";
  Print() << "=================================================\n\n";

  // Top boundary wall
  auto s_top = boxS(xlo, y_upper_hi, xhi, yhi);

  // Bottom walls - gap at LEFT exit (xs - W to xs)
  ...
  Print() << "[EB] ThreeBranch with left-exit vertical branch\n";
