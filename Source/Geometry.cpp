...
void ThreeBranch::build(const amrex::Geometry& geom, const int max_coarsening_level)
{
  ...
  Real cR  = 0.00;
  Real y_offset = 0.0;
  Real Z = 0.08;     // Vertical branch height (from input)
  Real zW = 0.005;   // Wall thickness (not used here but queried)
  ...
  // 🟡 Use Z to define vertical branch height directly
  const Real y_lower_lo = ylo + Z;
  ...
  Print() << "\n=== THREE-BRANCH GEOMETRY (Updated) ===\n";
  Print() << "Vertical branch: x=[" << z_x_left << ", " << z_x_right 
          << "], y=[" << ylo << ", " << y_lower_lo << "]\n";
  Print() << "Vertical branch height (Z): " << Z << "\n";
  Print() << "=======================================\n\n";

  // Top wall
  auto s_top = boxS(xlo, y_upper_hi, xhi, yhi);

  // Bottom walls (leave vertical channel opening)
  ...
  Print() << "[EB] ThreeBranch geometry complete.\n";
