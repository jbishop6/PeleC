...
void ThreeBranch::build(const amrex::Geometry& geom, const int max_coarsening_level)
{
  ...
  // Third branch: LEFT EDGE flush with exit (xr)
  ...
  Print() << "Vertical branch LEFT EDGE at exit: x=[" << z_x_left << ", " << z_x_right 
  ...
  // Bottom walls - gap starts at xr (exit position)
  ...
  // Two-branch system walls - RIGHT WALLS must stop at xr, not extend past
  auto s_left_upper   = boxS(xlo, y_base_hi, xs, y_upper_hi);
  auto s_left_lower   = boxS(xlo, y_lower_lo, xs, y_base_lo);
  auto s_right_upper  = boxS(xr, y_base_hi, xhi, y_upper_hi);
  auto s_right_lower  = boxS(xr, y_lower_lo, xhi, y_base_lo);  // Stops at xr
