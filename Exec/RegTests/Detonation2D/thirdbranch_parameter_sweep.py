...
SCRATCH_BASE = os.path.expanduser("~/PeleC/Exec/RegTests/Detonation2D/TB_PS")
os.makedirs(SCRATCH_BASE, exist_ok=True)
...
def main():
    # ------------------------------------------------------------
    # 1. Define which geometry parameters to sweep
    #    (edit these lists/ranges for your experiment)
    #    These correspond to the *third-branch* geometry in your .inp
    # ------------------------------------------------------------

    # Example: sweep Z, keep X/H/W fixed
    Z_values = np.linspace(0.08, 0.15, 9)   # <-- change range / # of points
    X_values = [0.326962268]                       # fixed X
    H_values = [0.188092301]                       # fixed H
    W_values = [0.084630218]                       # fixed W

    # If later you want a 2D sweep, give X_values or H_values multiple entries.

    # ------------------------------------------------------------
    # 2. Build list of valid geometry combinations
    #    NOTE: is_valid_input(x) expects (geo_X, geo_H, geo_Z, geo_W)
    # ------------------------------------------------------------
    combinations = []
    for Z in Z_values:
        for X in X_values:
            for H in H_values:
                for W in W_values:
                    if is_valid_input((X, H, Z, W)):
                        combinations.append((Z, X, H, W))
                    else:
                        print(
                            f"[WARN] Skipping invalid geometry: "
                            f"Z={Z:.4f}, X={X:.4f}, H={H:.4f}, W={W:.4f}"
                        )

    if not combinations:
        print("[FATAL] No valid geometry combinations to run. Check your ranges.")
        return

    print(f"[INFO] Running parameter sweep with {len(combinations)} cases")

    # ------------------------------------------------------------
    # 3. Loop over combinations and run PeleC
    # ------------------------------------------------------------
    for i, (Z, X, H, W) in enumerate(combinations, start=1):
        print("\n" + "=" * 60)
        print(f"[SWEEP {i}/{len(combinations)}] "
              f"Z={Z:.4f}, X={X:.4f}, H={H:.4f}, W={W:.4f}")
        print("=" * 60)
        ...
        try:
            thrust_stats = run_pelec_and_extract_thrust(
                ...
                iteration=i,       # will be written into RESULTS_LOG
            )

            print(
                f"[RESULT] Avg thrust = {thrust_stats['thrust_avg']:.3f} N, "
                f"Max thrust = {thrust_stats['thrust_max']:.3f} N"
            )

        except Exception as e:
            print(f"[ERROR] Sweep case {i} failed: {e}")
            # keep going with the rest of the sweep
            continue
